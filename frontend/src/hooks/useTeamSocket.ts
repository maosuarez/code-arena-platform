...
// Eventos dirigidos a todo el equipo (p.ej. inscripción a una competencia),
// independientes de cualquier competencia específica — ver useCompetitionSocket
// para eventos scoped a una competencia.
export function useTeamSocket(
  ...
  useEffect(() => {
    ...
    const topic = `${topicPrefix}/team/${teamCode}`
    ...
    client.on("error", (err) => {
      console.warn("[TeamSocket] MQTT error:", err.message)
      if (isFatalMqttError(err.message)) {
        console.error("[TeamSocket] MQTT auth no recuperable, deteniendo reconexión.")
        client.end(true)
      }
    })
